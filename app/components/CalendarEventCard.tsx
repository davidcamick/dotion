...
export default function CalendarEventCard({ type, eventId, summary, start, end, location, originalEvent, onUndo }: CalendarEventCardProps) {
  ...
  return (
    ...
    >
      {/* Glow effects */}
      <div className={`absolute -inset-0.5 bg-gradient-to-r ${config.from} via-current ${config.to} rounded-xl opacity-20 group-hover:opacity-40 blur transition duration-500`}></div>
      ...
      <div className="relative flex flex-col gap-3 p-5 bg-black/90 backdrop-blur-xl rounded-xl border border-white/10 shadow-2xl overflow-hidden">
        ...
        <div className={`absolute bottom-0 left-0 w-16 h-16 bg-${config.color}/10 rounded-tr-full blur-xl pointer-events-none`}></div>

        {/* Header */}
        <div className="flex items-start justify-between z-10">
           <div className="flex flex-col">
              <span className={`text-[10px] uppercase tracking-widest text-${config.color} font-bold mb-1 flex items-center gap-1`}>
                 <span className={`w-1.5 h-1.5 rounded-full bg-${config.color} animate-pulse`}></span>
                 {config.title}
              </span>
           ...
           <div className={`h-8 w-8 rounded-full bg-${config.color}/10 flex items-center justify-center border border-${config.color}/30 shrink-0 ml-2 text-${config.color}`}>
             {config.icon}
           </div>
        </div>
